...
def find_optimal_directory(dir, size, optimal_directory):
    for name, sub_dir in dir.items():
        if type(sub_dir) == dict:
            optimal_directory = find_optimal_directory(
                sub_dir, size, optimal_directory)
...
if __name__ == "__main__":
    ...
    with open("./day7/input.txt") as file:
        ...
        home = {next_line(file)[2]: add_directory(
            next_line(file, True), small_dirs)}
        # Print home directory (pretty though)
        print(json.dumps(home, indent=16))
    ...
    optimal_directory = find_optimal_directory(
        home, space_needed, [home_size, None])  # Size, name
